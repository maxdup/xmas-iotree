...
def usage():
    print(
        sys.argv[0] + " maximum_brightness(Default 100,  10-254) maximum_number_of_spheres(Default 5, 1-10)")


# if you are turning a lot of them on at once, keep their brightness down please
max_brightness = 100
# Min brightness for creating a new colour
min_brightness = math.floor(max_brightness/2)
...
if (len(sys.argv) == 2 or len(sys.argv) == 3) and sys.argv[1].isnumeric():
...
def random3DValues(min_values, max_values, not_like_this=[]):
    ...
    if hasattr(min_values, "__len__") and hasattr(max_values, "__len__") and (len(min_values) == len(max_values) == 3):
        ...
        while index < len(min_values):
            ...
            new_values.append(random.randint(
                min_values[index], max_values[index]))
            ...
            if len(not_like_this) == 3 and abs(new_values[index] - not_like_this[index]) < diff*0.1:
                ...
                new_values[index] = min_values[index] + \
                    (new_values[index] + diff/2) % diff
            index += 1
        return new_values
...
def createRandomGRBColour(not_like_this=[]):
    ...
    while index < 3:
        ...
        if len(not_like_this) == 3 and abs(new_colour[index] - not_like_this[index]) < max_brightness*0.1:
            # invert colour strength.
            new_colour[index] = (new_colour[index] +
                                 max_brightness/2) % max_brightness
...
def addColours(colour1, colour2):
    ...
    if hasattr(colour1, "__len__") and hasattr(colour2, "__len__") and (len(colour1) == len(colour2)):
        ...
        while index < len(colour1):
            new_colour.append(
                (colour1[index]*colour1[index] + colour2[index]*colour2[index]))
            brightness += new_colour[index]
            index += 1
        if brightness > max_brightness:
            index = 0
...
def xmaslight():
    ...
    for coord in coords:
        pixel_colour.append(createRandomGRBColour().copy())
    ...
    while index < number_of_spheres:
        ...
        # First is 0, rest Negative sizes so it won't start straight away
        sphere_rad_sizes.append(-index*max_size/number_of_spheres)
    ...
    while run:
        ...
        while index < number_of_spheres_to_use:
            ...
            if (sphere_rad_sizes[index] > max_size):
                # Negative sizes so it won't start straight away, and a bit random
                sphere_rad_sizes[index] = - \
                    (0.15*(random.random()+0.5)*(max_size))
                ...
                sphere_step_size[index] = random.gauss(
                    1, 0.25)*standard_step_size
                sphere_fade_modifier[index] = random.gauss(1, 0.25)
                sphere_wave_width[index] = random.gauss(
                    1, 0.25)*standard_wave_width
                cycles += 1
            ...
            while LED < len(coords):
                ...
                if sphere_rad_sizes[index]-norm < sphere_wave_width[index]*max_size*sphere_step_size[index] and sphere_rad_sizes[index]-norm > max_size*sphere_step_size[index]/sphere_wave_width[index]:
                    pixel_colour[LED] = addColours(
                        pixel_colour[LED], sphere_colours[index])
                # Else fade to black
                else:
                    pixel_colour[LED] = fadeColours(
                        pixel_colour[LED], black, sphere_fade_modifier[index]*number_of_spheres_to_use/2)
        ...
        while LED < len(coords):
            pixels[LED] = [math.floor(pixel_colour[LED][0]), math.floor(
                pixel_colour[LED][1]), math.floor(pixel_colour[LED][2])]
            LED += 1
